...
| Database / Storage | PostgreSQL | 주요 업무 데이터 저장 | 예정 |
| Database / Storage | pgvector | 벡터 임베딩 저장과 유사도 검색 | 예정 |
| Database / Storage | Redis | 캐시와 세션 또는 작업 큐 보조 | 예정 |
| Database / Storage | MinIO | 문서 파일 객체 저장 | 예정 |
| Infra | Docker Engine | 로컬 컨테이너 실행 | 예정 |
| Infra | Docker Compose | 로컬 통합 실행 환경 | 예정 |
