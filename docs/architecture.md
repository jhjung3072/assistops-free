...
- Agent UI: 세션 목록, 메시지 타임라인, assistant 출처와 latency 표시, 세션 삭제 구성
...
| Agent Chat UI | RAG Answer를 세션형 채팅 UX로 저장하고 조회 | 사용 중 |
...
11. Agent Chat 요청은 사용자 메시지를 저장한 뒤 기존 RAG Answer Service를 호출하고, assistant 메시지와 출처, latency metrics를 `agent_chat_*` 테이블에 저장합니다.
12. backend log에는 query embedding, vector search, prompt build, chat generation, persist 단계별 latency summary를 남깁니다.
13. 시스템 지표, 로그, trace는 OpenTelemetry 기반으로 수집하고 Prometheus, Loki, Grafana로 확인합니다.
...
현재 단계는 PostgreSQL 연결, JPA/Flyway 영속성 골격, JWT Bearer 인증 기반, workspace membership 기반 권한 골격, 프론트엔드 인증 화면, dashboard 초기 화면, 문서 업로드 및 원본 저장, 문서 텍스트 추출과 chunk 저장, Ollama embedding 생성, pgvector 유사 chunk 검색, Ollama chat model 기반 RAG 답변 생성, Agent Chat 세션 저장까지 다룹니다. Redis client와 queue 기반 비동기 처리는 아직 추가하지 않았습니다.
...
15. 목록/상세/다운로드/삭제/처리/chunk 조회/embedding/search/RAG/Agent API는 사용자가 속한 workspace 또는 본인 session에 대해서만 동작합니다.

현재 단계에서는 streaming answer, multi-turn context memory, tool calling, prompt versioning UI, workflow automation, release copilot을 수행하지 않습니다.
